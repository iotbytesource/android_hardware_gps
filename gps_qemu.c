...
//******************************************************************	
				...
				LOGE("%d",gps_fd);
                        ...
                        if (started) {
                            ...
                            started = 0;
//********************************************************************
				g_status.status=GPS_STATUS_SESSION_END; //停止
				state->callbacks.status_cb(&g_status); 
//********************************************************************
...
//*****************************************************************************
	state->fd= open("/dev/ttySAC3",O_RDWR|O_NOCTTY|O_NDELAY);//这里用的是UART1

     if( state->fd < 0){

            LOGE("open port /dev/ttySAC3 ERROR..state->fd=%d\n",state->fd); 

            exit(0);

        }else

            LOGE("open port:/dev/ttySAC3 succceed..state->fd=%d\n",state->fd);
    ...
    if (cfsetispeed(&gps_termios,B9600))//具体的参考GPS文档
...
qemu_gps_init(GpsCallbacks* callbacks)
{
	...
	s->init = 0;
    	...
    	//s.status = GPS_STATUS_NONE;

    	s->init = 1;	
/**************************************************/
 s->callbacks = *callbacks; //注册回调函数,JNI传下来的回调函数

    //g_status.status=GPS_STATUS_ENGINE_ON;//设置状态 通电但还没开始导航
